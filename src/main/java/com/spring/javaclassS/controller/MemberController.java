...
public class MemberController {
	...
	public String kakaoLoginGet(String nickName, String email, String accessToken,
		...
		session.setAttribute("sLoginNew", "kakao");
	...
	public String qrLoginCheckGet(Model model, HttpSession session,
		...
		if((intStrToday - intToday) > 5) return "redirect:/message/qrLoginTimeOver"; // 로그인 시간 제한
	...
	public String memberNaverLoginGet(HttpSession session, HttpServletRequest request, HttpServletResponse response,
		...
		session.setAttribute("sLoginNew", "naver");
		...
		// 현재 네이버로그인에 의한 우리회원이 아니였다면, 자동으로 우리회원에 가입처리한다.
		// 필수입력:아이디, 닉네임, 이메일, 성명(닉네임으로 대체), 비밀번호(임시비밀번호 발급처리)
		String newMember = "NO"; // 신규회원인지에 대한 정의(신규회원:OK, 기존회원:NO)
		if(vo == null) {
			// 아이디 결정하기
