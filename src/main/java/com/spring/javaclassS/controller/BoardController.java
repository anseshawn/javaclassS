...
public class BoardController {
	...
	public String boardInputPost(BoardVO vo) {
		...
		// 2. 이미지작업(복사작업)을 모두 마치면, ckeditor 폴더 경로를 board폴더 경로로 변경 처리한다.
	...
	public String boardUpdateGet(int idx, Model model,
		...
		// 수정화면으로 이동할 시에는 기존 게시글에 그림파일이 존재한다면, 현재 폴더(board)의 그림파일을 ckeditor 폴더로 복사시켜준다.
		BoardVO vo = boardService.getBoardContent(idx);
	...
	public String boardUpdatePost(BoardVO vo, Model model,
		...
		if(!origVo.getContent().equals(vo.getContent())) {
			// 1. 기존 board 폴더에 그림이 존재했다면 원본 그림을 모두 삭제처리한다.(원본 그림은 수정창에 들어오기 전에 ckeditor 폴더에 복사시켜두었다)
			...
			// 즉, content에 이미지가 저장되어 있다면, 저장된 이미지만 골라서 '/data/board/'폴더에 복사 저장 처리한다.
			if(vo.getContent().indexOf("src=\"/") != -1) boardService.imgCheck(vo.getContent());
			
			// 이미지들의 모든 복사 작업을 마치면, 폴더명을 'ckeditor'에서 'board'폴더로 변경 처리한다.
			vo.setContent(vo.getContent().replace("/data/ckeditor/", "/data/board/"));
		}
	...
	public String boardReplyInputPost(BoardReply2VO replyVO) {
		// 부모댓글의 경우는 re_step=0, re_order=1로 처리(단, 원본 글의 첫번 째 부모댓글은 re_order=1이지만 2번 이상은 마지막 부모댓글의 re_order보다 +1처리 시켜준다.
	...
	public String boardReplyInputRePost(BoardReply2VO replyVO) {
		// 1. 대댓글(답변글)의 re_step은 부모댓글의 re_step+1
		// 2. re_order는 부모의 re_order보다 큰 댓글은 모두 +1처리 후, 자신의 re_order+1 시켜준다.
		
		replyVO.setRe_step(replyVO.getRe_step()+1); // 1번
		
		boardService.setReplyOrderUpdate(replyVO.getBoardIdx(), replyVO.getRe_order()); // 2번(업데이트 필요)
	...
	public String boardSearchPost(Model model, String search,
		...
		else if(pageVO.getSearch().equals("nickName")) searchTitle = "작성자";
		else searchTitle = "내용";
