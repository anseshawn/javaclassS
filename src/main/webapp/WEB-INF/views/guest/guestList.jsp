...
<div class="container">
	...
	<table class="table table-borderless m-0 p-0">
		<tr>
			...
			<td><a href="${ctp}/guest/guestInput" class="btn btn-info">글쓰기</a></td>
			<td class="text-right">
				<c:if test="${pag > 1}">
					<a href="guestList?pag=1&pageSize=${pageSize}" title="처음"><i class="fa-solid fa-backward-fast" style="color:#2A93D5">&nbsp;</i></a>
					<a href="guestList?pag=${pag-1}&pageSize=${pageSize}" title="이전페이지"><i class="fa-solid fa-backward-step" style="color:#2A93D5">&nbsp;</i></a>
				</c:if>
				${pag}/${totPage}
				<c:if test="${pag < totPage}">
					<a href="guestList?pag=${pag+1}&pageSize=${pageSize}" title="다음페이지"><i class="fa-solid fa-forward-step" style="color:#2A93D5">&nbsp;</i></a>
					<a href="guestList?pag=${totPage}&pageSize=${pageSize}" title="끝"><i class="fa-solid fa-forward-fast" style="color:#2A93D5"></i></a>
				</c:if>
			</td>
		</tr>
		<tr>
...
<div class="text-center">
	<c:if test="${pag > 1}">[<a href="guestList?pag=1&pageSize=${pageSize}">처음</a>]</c:if>
	<c:if test="${curBlock > 0}">[<a href="guestList?pag=${(curBlock-1)*blockSize+1}&pageSize=${pageSize}">이전블록</a>]</c:if>
	<c:forEach var="i" begin="${(curBlock*blockSize+1)}" end="${(curBlock)*blockSize+blockSize}" varStatus="st">
		<%-- <c:if test="${}">[${i}]</c:if> --%>
		<c:if test="${i <= totPage && i == pag}"><b><a href="guestList?pag=${i}&pageSize=${pageSize}">[${i}]</a></b></c:if>
	...
	<c:if test="${pag < totPage}">[<a href="guestList?pag=${totPage}&pageSize=${pageSize}">끝</a>]</c:if>
</div>
<!-- 블록페이지 끝 -->
</div>
<p><br/></p>
